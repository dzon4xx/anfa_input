...
#include <util/delay.h>
 ...
 Frame::Frame()
{
	 this->pin = new Out_pin('b', 3);
	 this->led = new Out_pin('b', 4);
 	 this->listen_mode();
...
{
	 uint8_t in_byte = *(this->uart_port);
...
void Frame::send_mode(uint8_t response_frame_size)
{
	...
	this->led->high();
	*(this->uart_port) = this->data[this->send_data_pointer];
